...
Template.onRendered(function () {
  ...
  if (node[0] && node[0].nodeType !== 3) {
    // get rid of text nodes

    var template = self.view.name;
    var currentCount =
      Temple.instanceCount.get("Temple_render_count_" + template) || 1;
    var $node = node;

    var fix = $node.css("position") === "fixed" ? true : false;

    $node
      .addClass("is-template")
      .attr(
        "data-template",
        ($node.attr("data-template") ? $node.attr("data-template") + " " : "") +
          template +
          " (" +
          currentCount +
          ")"
    ...
    Temple.instanceCount.set(
      "Temple_render_count_" + template,
      currentCount + 1
    );
  }
});
...
Temple.makeBreadcrumbs = function (target) {
  ...
  var getName = function (view, name) {
    ...
    if (view && view.parentView && view.name.substr(0, 9) !== "Template.") {
      ...
      thisName = getName(
        view.parentView,
        view.parentView.name === "with" &&
          view.parentView.parentView &&
          view.parentView.parentView.name === "each"
          ? ""
  ...
  var viewName = getName(
    currentView,
    currentView
      ? currentView.name === "with" &&
        currentView.parentView &&
        currentView.parentView.name === "each"
        ? ""
        : currentView.name
      : "body"
  );
...
Template.body.events({
  "click, mouseover": function (evt) {
    if (
      Temple.dict.get("Temple_activated") ||
      (!!Constellation &&
        Constellation.isActive() &&
        Constellation.tabVisible("temple", "plugin"))
    ) {
      ...
      if (target.length && !$(target).closest("#Constellation").length) {
        ...
        if (evt.type === "click") {
          if (
            ...
            if (
              Temple.dict.get("Temple_activated") ||
              Constellation.isCurrentTab("temple", "plugin")
            ) {
...
Template.Temple_JSON.helpers({
  templeJSON: function () {
    var json = JSON.stringify(
      Temple.dict.get("Temple_current_context"),
      null,
      2
    );
    return (
      (!!Constellation &&
        Package["planable:console"].Constellation.colorize(json)) ||
      json
    );
  },
});

Template.Temple_JSON.events(
  Package["planable:console"].Constellation.foreignKeyLinkEvents
);